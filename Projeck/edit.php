...
// txt_reviw
            if (!isset($errorMsg)) {
                $update_stmt = $db->prepare("UPDATE tbl_file SET name = :name_up, image = :file_up ,reviw = :reviw_up WHERE id = :id");
                $update_stmt->bindParam(':name_up', $name);
                $update_stmt->bindParam(':reviw_up', $reviw);
...
<body>
    ...
    <div class="container text-center">
        ...
        <div class="form-group">
            <div class="row">
            <label for="name" class="col-sm-3 control-label">รีวิว</label>
            <div class="col-sm-9">
                <input type="text" name="txt_reviw" class="form-control" value="<?php echo $reviw; ?>">
            </div>
            </div>
        </div>
